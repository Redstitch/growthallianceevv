...
const PostWidgets = ({ preview, content, color }) => content && content.map((widget, index) => (
  <section key={`${index}postwidgets`}>
    {preview ? (
      <>
        {widget.acf_fc_layout === 'image_block' && <Wrapper narrow><FullWidthImage order={index} widget={widget} color={color} /></Wrapper> }
