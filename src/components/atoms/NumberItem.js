...
class NumberItem extends Component {
  ...
  render() {
    const { content, alignment, color, order, rowColor } = this.props;
        ...
        >
          <span>
            <CountUp
              ...
              suffix={
                content.prefix__suffix === '%' ? content.prefix__suffix : null
              }
              prefix={
                content.prefix__suffix === '$' || content.prefix__suffix === '#'
                  ? content.prefix__suffix
                  : null
              }
            />
          </span>
          <h5>{content.label}</h5>
          {content.sub_copy && (
            <span className="sub-copy">{content.sub_copy}</span>
          )}
        </SNumberItem>
      </VisibilitySensor>
    );
...
export const SNumberItem = styled.div`
  ...
  flex-direction: ${({ alignment }) =>
    alignment === 'top' ? 'column' : 'column-reverse'};
  position: relative;
  ...
  span {
    color: ${({ color, rowColor }) =>
      numberColor(color, rowColor, colors.green)};
    font-size: 60px;
  ...
  h5 {
    color: ${colors.white};
    background-color: ${({ color, rowColor }) =>
      numberColor(color, rowColor, colors.green)};
  ...
  &:nth-child(4n - 2) {
    span {
      color: ${({ color, rowColor }) =>
        numberColor(color, rowColor, colors.orange)};
    }
  ...
  &:nth-child(4n - 1) {
    span {
      color: ${({ color, rowColor }) =>
        numberColor(color, rowColor, colors.blue)};
    }
  ...
  &:nth-child(4n) {
    span {
      color: ${({ color, rowColor }) =>
        numberColor(color, rowColor, colors.navy)};
    }
